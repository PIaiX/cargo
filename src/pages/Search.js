...
export default function Search() {
  ...
  const submitHandler = (formData) =>
    formData
      ? setFilters({ ...initialFilters, ...formData })
  ...
  useEffect(() => {
    searchType === "car" &&
      searchRoute(carsPagination.currentPage, carsPagination.pageLimit, {
        ...
        .catch((error) =>
          setCars((prev) => ({ ...prev, isLoading: true, error }))
        );
          ...
          <div className="fs-12 ms-md-5 d-flex align-items-center">
            ...
            <Select
              ...
              value={
                ...
                initialSorting.filter((item) => item.value === filters.orderBy)
              }
              onChange={(val) =>
                setFilters((prev) => ({ ...prev, orderBy: val.value }))
              }
            />
          </div>
        </div>
