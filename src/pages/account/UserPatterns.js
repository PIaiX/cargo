...
export default function UserPatterns() {
  ...
  const [isShowModalDeleteTemplateRoute, setIsShowModalDeleteTemplateRoute] =
    useState(false);
  ...
  useEffect(() => {
    (currentUser?.roleId === 3 || currentUser?.roleId === 4) &&
      getTemplates(
        ...
        .catch((error) =>
          setRoutesTemplates((prev) => ({ ...prev, isLoading: true, error }))
        );
  ...
  useEffect(() => {
    (currentUser?.roleId === 2 || currentUser?.roleId === 4) &&
      getCargoTemplates(
        ...
        .catch((error) =>
          setCargoTemplates((prev) => ({ ...prev, isLoading: true, error }))
        );
  ...
  const getFirstRouteForCargo = (templateId) => {
    ...
    const template = cargoTemplates?.data.find(
  ...
  const getCargoInfo = (templateId) => {
    ...
    const cargoWeight = template?.cargo.items.reduce(
